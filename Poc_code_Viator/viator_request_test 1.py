...
Updated version of viator_request_test 1.py with MongoDB integration and cache management
...
def change_currency(page, currency):
    """Change the currency on the page"""
    try:
        page.locator("//button[@data-automation='language_currency_button']").click()
        page.locator("//button[@data-automation='currency']").click()
        page.locator(
    ...
    try:
        ...
        if not html_exists:
            ...
            if current_country != sm:
                currency = CURRENCY_BY_COUNTRY.get(sm, "USD")
        ...
        if not json_exists:
            ...
            # Navigate to page if we haven't already
            if html_exists:
                page.goto(referer_url, wait_until="domcontentloaded", timeout=60000)
                page.wait_for_timeout(2000)
